...
Para isso, restringimos as variáveis aos inteiros,  \DIFaddbegin \DIFadd{tornando-o }\DIFaddend assim um \emph{Programa Linear Inteiro}. Caso apenas um subconjunto das variáveis possuam a restrição de integralidade, chamamos esse programa de linear misto.
...
\DIFaddbegin \DIFadd{Uma }\emph{\DIFadd{faceta}} \DIFadd{pode ser definida como uma face própria maximal, ou seja, uma face própria que não está contida em nenhuma outra.
}\DIFaddend 
...
Neste algoritmo, a decisão de qual variável será adicionada é feita pelo subproblema de geração de coluna \DIFaddbegin\DIFadd{caracterizado por um }\DIFaddend \emph{Problema de Conjunto Independente de Peso Máximo}:
\begin{alignat*}{4}
...
Um algoritmo de \emph{planos-de-corte} é aquele que parte de um modelo relaxado, resolve-se e iterativamente acrescenta-se inequações válidas, chamadas de planos-de-corte,  \DIFaddbegin \DIFadd{para }\DIFaddend eliminar tal solução.
