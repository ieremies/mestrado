...
Muitas vezes, deseja\DIFaddbegin\DIFadd{mos }\DIFaddend alocar eventos ou recursos sem que hajam conflitos, seja na divisão de tarefas num cenário de fábricas ou escritórios, ou na organização de eventos.
...
Tal problema pode ser convertido a um de coloração transformando cada evento a um vértice no qual dois vértices estão ligados se há necessidade de ao menos uma pessoa estar em ambos eventos. \DIFaddbegin \DIFadd{Em uma }\DIFaddend solução, cada cor irá representar um horário para os eventos acontecerem.

Além dessas, como discutido por \textcite{Malaguti2010surveyvertexcoloring}, diversas são as aplicações do problema, como: agendamento \autocite{Leighton1979GraphColoringAlgorithm}, tabela de horários \autocite{Werra1985introductiontimetabling}, alocação de registradores \autocite{Chow1990prioritybasedcoloring}, comunicação de redes \autocite{Caprara2007PassengerRailwayOptimization} e alocação de banda \autocite{Gamst1986Somelowerbounds}.
...
\DIFaddbegin \DIFadd{Recentemente, também observamos avanços significativos utilizando técnicas de enumeração e }\emph{\DIFadd{SAT}} \DIFadd{por \mbox{%DIFAUXCMD
...
Em geral, as instâncias usadas para os resultados de experimentos computacionais são as apresentadas  \DIFaddbegin \DIFadd{em \mbox{%DIFAUXCMD
...
Neste algoritmo, a decisão de qual variável será adicionada é feita pelo subproblema de geração de coluna \DIFaddbegin\DIFadd{caracterizado pelo }\DIFaddend \emph{Problema de Conjunto Independente de Peso Máximo}:
\begin{alignat*}{4}
...
\DIFaddbegin \DIFadd{\textcite[]{Held2012Maximumweightstable} propõem utilizar o algoritmo de \textcite[]{Oestergaard1999NewAlgorithmMaximum} para encontrar conjuntos estáveis de peso máximo em grafos densos. Para os demais, os autores apresentam um algoritmo de enumeração baseado em \emph{branch-and-bound} que guarda, a cada nó, o maior conjunto estável encontrado, os vértices que ainda podem ser utilizados e aqueles que já foram descartados para formar um conjunto estável. Os autores utilizam do fato de que, se há algum vértice $x$ descartado cujo peso é maior do que qualquer conjunto a ser formado sem a vizinhança de $x$ naquele subproblema, ele não precisa ser expandido. Por fim, eles mostram que podemos obter uma $\frac{n}{k}$-aproximação numericamente segura, tal que $k$ é a quantidade de números inteiros que podemos representar com a quantidade de bits utilizados por inteiro (por exemplo, $2^{32}$ para uma arquitetura de $32$ bits). Os resultados de \textcite[]{Held2012Maximumweightstable} são considerados estado da arte em \emph{branch-and-price} para o nosso problema.}

\DIFaddend Em caso de soluções fracionárias,  \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
Além disso, eles usam \emph{propagação de etiquetas}, técnica comum na resolução de problemas de precificação  que pode ser interessante para o nosso problema.
 ...
 \DIFaddbegin \DIFadd{Tendo em vista que tais técnicas ainda não foram consideradas para o }\DIFaddend problema de coloração\DIFaddbegin \DIFadd{, objetivamos principalmente aplicá-las, bem como desenvolver qualquer necessidade de modificação ou melhoria}\DIFaddend .
...
conseguiu avanços significativos em sua tese.}\DIFaddend

\DIFaddbegin \DIFadd{Esperamos também ter que desenvolver novas técnicas específicas para o problema de coloração de vértices, uma vez que a aplicação das técnicas mencionadas para outros problemas podem abrir caminhos e ideias para métodos ainda melhores. }
...
Os resultados dos experimentos computacionais serão comparados utilizando técnicas como \emph{Performance Profile} apresentado por \textcite{Dolan2002Benchmarkingoptimizationsoftware}.
