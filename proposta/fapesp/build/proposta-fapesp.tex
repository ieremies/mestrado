...
\maketitle
\begin{abstract}
A coloração em grafos é um clássico problema da literatura que, apesar disso, não deixa de ser relevante em problemas cada vez mais atuais.
Determinar uma organização de recursos sem que haja certos conflitos se mostra um desafio em divsersas áreas e resolve-lo de forma eficiente é objeto de estudo de diversas áreas.

Nesse projeto, objetivamos utilizar Programação Linear Inteira para avançar os resultados já apresentados tanto por esta técnica como por outras.
Acreditamos que nosso trabalho se diferencia ao utilizar avanços nos últimos anos em métodos como \emph{Branch-and-Price} e planos de corte.

Dessa forma, objetivamos resultados competitivos nas instâncias consolidadas na literatura, bem como possíveis novas formulações, cortes e estratégias para melhor resolve-las.
\end{abstract}

\section{Introdução}
\label{sec:org46a3325}
...
\section{Metodologia}
\label{sec:org7710e79}
\label{metodologia}
\subsection{Programação Linear}
\label{sec:orgb0f8ceb}
...
Definimos um \textbf{poliedro} como um subconjunto \(P \subseteq \mathbb{R}^n\) tal que ele possa ser descrito por meio de inequações em tal espaço, similar aos programas lineares apresentados acima.
...
Um algoritmo de \emph{branch-and-bound} consiste em, durante a árvore de \emph{branch-and-bound}, adicionar inequalidades e cortes válidos para diminuir o espaço de busca. Um corte válido é uma inequação que não elimina nenhuma solução inteira viável do nosso modelo.
...
Como \textcite{Campelo2008AsymmetricRepresentativesFormulation} discutem, existem diversas soluções simétricas, apenas distinguem no representante das cores sem alterar efetivamente a solução.
Eles propõem acrescentar uma ordenação para que apenas o menor vértice possa ser o representante, porém este modelo possui um número exponencial de variáveis e portanto requer técnicas como /branch-and-price /, abordada mais afundo na Seção \ref{conjuntos_independentes}.
...
\label{sec:org18701f7}
Apresentado inicialmente por \textcite{Jabrayilov2018NewIntegerLinear} e posteriormente melhorado \autocite{Jabrayilov2022StrengthenedPartialOrdering}, utilizamos um misto do modelo de atribuição e a ordenação parcial da união entre os vértices e as cores disponíveis.
...
\label{sec:org0a3a623}
Por fim, mais recentemente, \textcite{Hoeve2021Graphcoloringdecision} aplicaram técnicas de diagrama de decisão.
...
\autocite{Hoeve2021Graphcoloringdecision} utilizam a formulação de conjuntos independentes para criar aplicar tal técnica, onde cada variável indica o uso, ou não, de um vértice num conjunto.
...
\hline
Disciplinas & x & x & x &  &  &  &  & \\[0pt]
EQM &  & x &  &  &  &  &  & \\[0pt]
PED &  &  & x &  &  &  &  & \\[0pt]
Revi. Biblio. & x & x & x &  &  &  &  & \\[0pt]
Implem. estado-da-arte &  &  & x &  &  &  &  & \\[0pt]
Desenvolvimento &  &  & x & x & x & x & x & x\\[0pt]
Escrita &  &  &  &  &  &  & x & x\\[0pt]
Intercâmbio &  &  &  &  & x & x &  & \\[0pt]
\end{tabular}
\end{table}
