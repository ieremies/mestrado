...
# PROF: tá super genérico, seja mais direto, fale com leitor de computação
#+begin_abstract
...
Determinar uma organização de recursos sem que haja certos conflitos se mostra um desafio em diversas áreas e resolvê-lo de forma eficiente é objeto de estudo de diversas áreas.

Nesse projeto, objetivamos utilizar Programação Linear Inteira para avançar os resultados tanto por desta técnica como de outras.
Acreditamos que nosso trabalho se diferencia ao utilizar avanços nos últimos anos em métodos como /Branch-and-Price/ e planos de corte.
...
Decidir se um grafo possui uma coloração própria que usa até $k \geq 3$ cores é um problema NP-completo [cite:@Garey1979ComputersIntractabilityGuide].
Como discutido por [cite/text:@Lewis2015GuideGraphColouring], o problema de otimização também é NP-difícil.
# PROF: o prob é NP-difícil e GJ prova que a 3-coloração é NP-completo
# eu não encontrei uma referência que mostra que o problema de otimização é NP-hard.
# Tem referências ao Lewis 2015, mas nele ele só diz que o problema de otimização é NP-hard, sem uma prova formal...
...
# PROF: deixe claro que é em termos de tempo
...
# PROF: existem outros, melhor citar.
# eu não conferi essa citação mas não achei melhor.
...
# PROF: é pior do que isso, não? n! / (n-k)!
a. Muitas soluções são simétricas umas às outras, já que as cores são indistinguíveis. Uma solução que utiliza $k$ cores possui $k$ permutações de cores do que é, efetivamente, a mesma solução.
b. A relaxação linear do modelo é extremamente fraca.
# PROF: isto é... explicar o que isso significa.

# PROF: como eu junto essas duas referências??
...
# PROF: conjunto independente não é definido
...
# PROF: podia ter definido isso quando definiu face
...
# PROF: Não entendi de onde vem esse >
...
# PROF: duas marcações que eu não entendi
...
Além disso, pretendemos utilizar técnicas de análise estatísica como apresentadas por [cite:@Derrac2011practicaltutorialuse] afim de produzir comparações mais rigorozas entre as propostas desse projeto.
